    ...
    <ScrollArea className="h-full px-3 py-3">
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <Checkbox
            checked={cut.enabled}
            onCheckedChange={(v) => viewer?.enableCut(Boolean(v))}
        ...
        <div className="mt-3 flex gap-2">
          <Button
            variant={cut.orientation === "horizontal" ? "default" : "outline"}
            size="sm"
            disabled={!cut.enabled || isTouchDevice}
            onClick={() => viewer?.setCutOrientation("horizontal")}
          >
            Horizontal
          </Button>
          <Button
            variant={cut.orientation === "vertical" ? "default" : "outline"}
            size="sm"
            disabled={!cut.enabled || isTouchDevice}
            onClick={() => viewer?.setCutOrientation("vertical")}
          >
            Vertical
          </Button>
        </div>
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          ...
          <Checkbox
            checked={cut.flip}
            onCheckedChange={(v) => viewer?.setCutFlip(Boolean(v))}
            disabled={!cut.enabled || isTouchDevice}
          />
        </div>
        ...
        <div className="mt-2">
          ...
          <div className="mt-1 text-[11px] text-slate-500">
            Tip: activate <span className="font-medium">Cut</span> tool and drag directly in the model to scrub the
            plane.
          </div>
          <div className="mt-0.5 text-[11px] text-slate-500">Shortcuts: `V` toggle axis, `[` and `]` nudge.</div>
        </div>
        ...
        {isTouchDevice ? (
          <div className="mt-2 text-[11px] text-slate-500">Advanced tools available on desktop</div>
        ) : null}
      </div>
      ...
      <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2 flex gap-2">
          <Button
            variant={propertyFilter.mode === "show" ? "default" : "outline"}
            size="sm"
            onClick={() => setFilter({ mode: "show" })}
          >
            Show only
          </Button>
          <Button
            ...
            classGroups.slice(0, 40).map((g) => (
              <div key={g.id} className="flex items-center justify-between gap-2">
                ...
                <Checkbox
                  ...
                  onCheckedChange={(v) => void toggleClass(g.id, Boolean(v))}
                />
              </div>
            ))
          )}
            ...
            storeyGroups.map((g) => (
              <div key={g.id} className="flex items-center justify-between gap-2">
                ...
                <Checkbox
                  ...
                  onCheckedChange={(v) => void toggleStorey(g.id, Boolean(v))}
                />
              </div>
            ))
          )}
